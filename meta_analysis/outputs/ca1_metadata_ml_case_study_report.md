...
The TouchAndSee dataset is currently represented mainly by file/session metadata, because Neo/pickle loading failed in the provided extraction JSON. Once the legacy pickle compatibility issue is fixed and the extraction is rerun, the same pipeline will include its internal electrophysiology metadata automatically if present in the JSON.
